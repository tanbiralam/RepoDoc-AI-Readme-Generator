...
import { signInWithEmail, signInWithGitHub, signInWithGoogle, signUpWithEmail } from '@/services/auth';
...
export default function AuthPage() {
  ...
  const handleGitHubSignIn = async () => {
    setError(null);
    try {
      // Request only public access initially
      const { error } = await signInWithGitHub(['read:user', 'user:email']);
      if (error) throw error;
    } catch (err) {
      setError(err instanceof Error ? err.message : 'GitHub authentication failed. Please try again.');
    }
  };
